"""Example of creating a custom device adapter."""
...
class SmartCoffeeMakerAdapter(DeviceAdapter):
    """Adapter for a smart coffee maker device."""
    
    SUPPORTED_COMMANDS = [
        'turnOn', 'turnOff', 'brew', 'setStrength', 
        'setSize', 'getStatus', 'clean'
    ]
...
if __name__ == '__main__':
    ...
    coffee_maker = SmartCoffeeMakerAdapter('coffee1', 'Kitchen Coffee Maker')
    manager.register_device(coffee_maker)
    
    # Create and run app
    app = create_app(manager)
    
    print("Smart Coffee Maker registered!")
    print("Try these commands:")
    print("  POST /api/v1/devices/coffee1/command")
    print("    {'command': 'turnOn'}")
    print("    {'command': 'setStrength', 'parameters': {'strength': 'strong'}}")
    print("    {'command': 'brew'}")
